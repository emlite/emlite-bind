...
impl FromVal for ClipboardItem {
    fn from_val(v: &Any) -> Self {
        ClipboardItem { inner: Any::from_val(v) }
    }
...
impl ClipboardItem {
    ...
    pub fn new0(items: &Record<JsString, Any>) -> ClipboardItem {
        Self {
            inner: Any::global("ClipboardItem").new(&[items.into()]).as_::<Any>(),
        }
    }
    ...
    pub fn new1(items: &Record<JsString, Any>, options: &ClipboardItemOptions) -> ClipboardItem {
        Self {
            inner: Any::global("ClipboardItem").new(&[items.into(), options.into()]).as_::<Any>(),
        }
    }
...
impl ClipboardItem {
    ...
    pub fn get_type(&self, type_: &JsString) -> Promise<Blob> {
        self.inner.call("getType", &[type_.into(), ]).as_::<Promise<Blob>>()
    }
}
impl ClipboardItem {
    /// The supports method.
    ...
    pub fn supports(type_: &JsString) -> bool {
        Any::global("ClipboardItem").call("supports", &[type_.into(), ]).as_::<bool>()
    }
}
