...
impl FromVal for MIDIConnectionEvent {
    fn from_val(v: &Any) -> Self {
        MIDIConnectionEvent { inner: Event::from_val(v) }
    }
...
impl MIDIConnectionEvent {
    ...
    pub fn new0(type_: &JsString) -> MIDIConnectionEvent {
        Self {
            inner: Any::global("MIDIConnectionEvent").new(&[type_.into()]).as_::<Event>(),
        }
    }

    /// The `new MIDIConnectionEvent(..)` constructor, creating a new MIDIConnectionEvent instance
    pub fn new1(type_: &JsString, event_init_dict: &MIDIConnectionEventInit) -> MIDIConnectionEvent {
        Self {
            inner: Any::global("MIDIConnectionEvent").new(&[type_.into(), event_init_dict.into()]).as_::<Event>(),
        }
    }
