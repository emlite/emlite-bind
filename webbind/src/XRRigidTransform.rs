...
impl FromVal for XRRigidTransform {
    fn from_val(v: &emlite::Val) -> Self {
        XRRigidTransform { inner: emlite::Val::from_val(v) }
    }
...
impl XRRigidTransform {
    pub fn new0() -> XRRigidTransform {
        Self {
            inner: emlite::Val::global("XRRigidTransform").new(&[]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(position: DOMPointInit) -> XRRigidTransform {
        Self {
            inner: emlite::Val::global("XRRigidTransform").new(&[position.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new2(position: DOMPointInit, orientation: DOMPointInit) -> XRRigidTransform {
        Self {
            inner: emlite::Val::global("XRRigidTransform").new(&[position.into(), orientation.into()]).as_::<emlite::Val>(),
        }
    }
