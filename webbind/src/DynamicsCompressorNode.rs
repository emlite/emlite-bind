...
impl DynamicsCompressorNode {
    pub fn new0(context: BaseAudioContext) -> DynamicsCompressorNode {
        Self {
            inner: emlite::Val::global("DynamicsCompressorNode").new(&[context.into()]).as_::<AudioNode>(),
        }
    }

    pub fn new1(context: BaseAudioContext, options: Any) -> DynamicsCompressorNode {
        Self {
            inner: emlite::Val::global("DynamicsCompressorNode").new(&[context.into(), options.into()]).as_::<AudioNode>(),
        }
    }
