...
impl FromVal for Request {
    fn from_val(v: &emlite::Val) -> Self {
        Request { inner: emlite::Val::from_val(v) }
    }
...
impl Request {
    pub fn new0(input: Any) -> Request {
        Self {
            inner: emlite::Val::global("Request").new(&[input.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(input: Any, init: Any) -> Request {
        Self {
            inner: emlite::Val::global("Request").new(&[input.into(), init.into()]).as_::<emlite::Val>(),
        }
    }
