...
impl BiquadFilterNode {
    pub fn new0(context: BaseAudioContext) -> BiquadFilterNode {
        Self {
            inner: emlite::Val::global("BiquadFilterNode").new(&[context.into()]).as_::<AudioNode>(),
        }
    }

    pub fn new1(context: BaseAudioContext, options: Any) -> BiquadFilterNode {
        Self {
            inner: emlite::Val::global("BiquadFilterNode").new(&[context.into(), options.into()]).as_::<AudioNode>(),
        }
    }
