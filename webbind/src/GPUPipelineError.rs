...
impl GPUPipelineError {
    ...
    pub fn new0() -> GPUPipelineError {
        Self {
            inner: Any::global("GPUPipelineError").new(&[]).as_::<DOMException>(),
        }
    }
    ...
    pub fn new1(message: &JsString) -> GPUPipelineError {
        Self {
            inner: Any::global("GPUPipelineError").new(&[message.into()]).as_::<DOMException>(),
        }
    }
    ...
    pub fn new2(message: &JsString, options: &GPUPipelineErrorInit) -> GPUPipelineError {
        Self {
            inner: Any::global("GPUPipelineError").new(&[message.into(), options.into()]).as_::<DOMException>(),
        }
    }
