...
impl PaymentResponse {
    ...
    pub fn complete1(&self, result: &PaymentComplete) -> Promise<Undefined> {
        self.inner.call("complete", &[result.into(), ]).as_::<Promise<Undefined>>()
    }
    /// The complete method.
    /// [`PaymentResponse.complete`](https://developer.mozilla.org/en-US/docs/Web/API/PaymentResponse/complete)
    pub fn complete2(&self, result: &PaymentComplete, details: &PaymentCompleteDetails) -> Promise<Undefined> {
        self.inner.call("complete", &[result.into(), details.into(), ]).as_::<Promise<Undefined>>()
    }
}
impl PaymentResponse {
    /// The retry method.
    ...
    pub fn retry1(&self, error_fields: &PaymentValidationErrors) -> Promise<Undefined> {
        self.inner.call("retry", &[error_fields.into(), ]).as_::<Promise<Undefined>>()
    }
}
impl PaymentResponse {
