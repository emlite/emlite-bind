...
impl FromVal for DOMPointReadOnly {
    fn from_val(v: &emlite::Val) -> Self {
        DOMPointReadOnly { inner: emlite::Val::from_val(v) }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
...
impl DOMPointReadOnly {
    pub fn new0() -> DOMPointReadOnly {
        Self {
            inner: emlite::Val::global("DOMPointReadOnly").new(&[]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(x: f64) -> DOMPointReadOnly {
        Self {
            inner: emlite::Val::global("DOMPointReadOnly").new(&[x.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new2(x: f64, y: f64) -> DOMPointReadOnly {
        Self {
            inner: emlite::Val::global("DOMPointReadOnly").new(&[x.into(), y.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new3(x: f64, y: f64, z: f64) -> DOMPointReadOnly {
        Self {
            inner: emlite::Val::global("DOMPointReadOnly").new(&[x.into(), y.into(), z.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new4(x: f64, y: f64, z: f64, w: f64) -> DOMPointReadOnly {
        Self {
            inner: emlite::Val::global("DOMPointReadOnly").new(&[x.into(), y.into(), z.into(), w.into()]).as_::<emlite::Val>(),
        }
    }
...
impl DOMPointReadOnly {
    pub fn from_point0() -> DOMPointReadOnly {
        emlite::Val::global("dompointreadonly").call("fromPoint", &[]).as_::<DOMPointReadOnly>()
    }

    pub fn from_point1(other: DOMPointInit) -> DOMPointReadOnly {
        emlite::Val::global("dompointreadonly").call("fromPoint", &[other.into(), ]).as_::<DOMPointReadOnly>()
    }
...
impl DOMPointReadOnly {
    ...
    pub fn matrix_transform1(&self, matrix: DOMMatrixInit) -> DOMPoint {
        self.inner.call("matrixTransform", &[matrix.into(), ]).as_::<DOMPoint>()
    }
