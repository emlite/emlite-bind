...
impl FromVal for URL {
    fn from_val(v: &emlite::Val) -> Self {
        URL { inner: emlite::Val::from_val(v) }
    }
...
impl URL {
    pub fn new0(url: USVString) -> URL {
        Self {
            inner: emlite::Val::global("URL").new(&[url.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(url: USVString, base: USVString) -> URL {
        Self {
            inner: emlite::Val::global("URL").new(&[url.into(), base.into()]).as_::<emlite::Val>(),
        }
    }
...
impl URL {
    pub fn parse0(url: USVString) -> URL {
        emlite::Val::global("url").call("parse", &[url.into(), ]).as_::<URL>()
    }

    pub fn parse1(url: USVString, base: USVString) -> URL {
        emlite::Val::global("url").call("parse", &[url.into(), base.into(), ]).as_::<URL>()
    }

}
impl URL {
    pub fn can_parse0(url: USVString) -> bool {
        emlite::Val::global("url").call("canParse", &[url.into(), ]).as_::<bool>()
    }

    pub fn can_parse1(url: USVString, base: USVString) -> bool {
        emlite::Val::global("url").call("canParse", &[url.into(), base.into(), ]).as_::<bool>()
    }
...
impl URL {
    pub fn create_object_url(obj: Any) -> DOMString {
        emlite::Val::global("url").call("createObjectURL", &[obj.into(), ]).as_::<DOMString>()
    }

}
impl URL {
    pub fn revoke_object_url(url: DOMString) -> Undefined {
        emlite::Val::global("url").call("revokeObjectURL", &[url.into(), ]).as_::<Undefined>()
    }
