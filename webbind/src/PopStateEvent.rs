use super::*;

#[derive(Clone, Debug)]
pub struct PopStateEvent {
    inner: Event,
}
impl FromVal for PopStateEvent {
    fn from_val(v: &emlite::Val) -> Self {
        PopStateEvent {
            inner: Event::from_val(v),
        }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
    fn as_handle(&self) -> emlite::env::Handle {
        self.inner.as_handle()
    }
}
impl std::ops::Deref for PopStateEvent {
    type Target = Event;
    fn deref(&self) -> &Self::Target {
        &self.inner
    }
}
impl std::ops::DerefMut for PopStateEvent {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.inner
    }
}
impl From<PopStateEvent> for emlite::Val {
    fn from(s: PopStateEvent) -> emlite::Val {
        let handle = s.inner.as_handle();
        std::mem::forget(s);
        emlite::Val::take_ownership(handle)
    }
}

impl PopStateEvent {
    pub fn new0(type_: jsbind::DOMString) -> PopStateEvent {
        Self {
            inner: emlite::Val::global("PopStateEvent")
                .new(&[type_.into()])
                .as_::<Event>(),
        }
    }

    pub fn new1(type_: jsbind::DOMString, event_init_dict: jsbind::Any) -> PopStateEvent {
        Self {
            inner: emlite::Val::global("PopStateEvent")
                .new(&[type_.into(), event_init_dict.into()])
                .as_::<Event>(),
        }
    }
}
impl PopStateEvent {
    pub fn state(&self) -> jsbind::Any {
        self.inner.get("state").as_::<jsbind::Any>()
    }
}
impl PopStateEvent {
    pub fn has_ua_visual_transition(&self) -> bool {
        self.inner.get("hasUAVisualTransition").as_::<bool>()
    }
}
