use super::*;

#[derive(Clone, Debug)]
pub struct UIEvent {
    inner: Event,
}
impl FromVal for UIEvent {
    fn from_val(v: &emlite::Val) -> Self {
        UIEvent {
            inner: Event::from_val(v),
        }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
    fn as_handle(&self) -> emlite::env::Handle {
        self.inner.as_handle()
    }
}
impl std::ops::Deref for UIEvent {
    type Target = Event;
    fn deref(&self) -> &Self::Target {
        &self.inner
    }
}
impl std::ops::DerefMut for UIEvent {
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.inner
    }
}
impl From<UIEvent> for emlite::Val {
    fn from(s: UIEvent) -> emlite::Val {
        let handle = s.inner.as_handle();
        std::mem::forget(s);
        emlite::Val::take_ownership(handle)
    }
}

impl UIEvent {
    pub fn new0(type_: jsbind::DOMString) -> UIEvent {
        Self {
            inner: emlite::Val::global("UIEvent")
                .new(&[type_.into()])
                .as_::<Event>(),
        }
    }

    pub fn new1(type_: jsbind::DOMString, event_init_dict: jsbind::Any) -> UIEvent {
        Self {
            inner: emlite::Val::global("UIEvent")
                .new(&[type_.into(), event_init_dict.into()])
                .as_::<Event>(),
        }
    }
}
impl UIEvent {
    pub fn view(&self) -> Window {
        self.inner.get("view").as_::<Window>()
    }
}
impl UIEvent {
    pub fn detail(&self) -> i32 {
        self.inner.get("detail").as_::<i32>()
    }
}
impl UIEvent {
    pub fn source_capabilities(&self) -> InputDeviceCapabilities {
        self.inner
            .get("sourceCapabilities")
            .as_::<InputDeviceCapabilities>()
    }
}
impl UIEvent {
    pub fn init_ui_event0(&self, type_arg: jsbind::DOMString) -> jsbind::Undefined {
        self.inner
            .call("initUIEvent", &[type_arg.into()])
            .as_::<jsbind::Undefined>()
    }

    pub fn init_ui_event1(
        &self,
        type_arg: jsbind::DOMString,
        bubbles_arg: bool,
    ) -> jsbind::Undefined {
        self.inner
            .call("initUIEvent", &[type_arg.into(), bubbles_arg.into()])
            .as_::<jsbind::Undefined>()
    }

    pub fn init_ui_event2(
        &self,
        type_arg: jsbind::DOMString,
        bubbles_arg: bool,
        cancelable_arg: bool,
    ) -> jsbind::Undefined {
        self.inner
            .call(
                "initUIEvent",
                &[type_arg.into(), bubbles_arg.into(), cancelable_arg.into()],
            )
            .as_::<jsbind::Undefined>()
    }

    pub fn init_ui_event3(
        &self,
        type_arg: jsbind::DOMString,
        bubbles_arg: bool,
        cancelable_arg: bool,
        view_arg: Window,
    ) -> jsbind::Undefined {
        self.inner
            .call(
                "initUIEvent",
                &[
                    type_arg.into(),
                    bubbles_arg.into(),
                    cancelable_arg.into(),
                    view_arg.into(),
                ],
            )
            .as_::<jsbind::Undefined>()
    }

    pub fn init_ui_event4(
        &self,
        type_arg: jsbind::DOMString,
        bubbles_arg: bool,
        cancelable_arg: bool,
        view_arg: Window,
        detail_arg: i32,
    ) -> jsbind::Undefined {
        self.inner
            .call(
                "initUIEvent",
                &[
                    type_arg.into(),
                    bubbles_arg.into(),
                    cancelable_arg.into(),
                    view_arg.into(),
                    detail_arg.into(),
                ],
            )
            .as_::<jsbind::Undefined>()
    }
}
impl UIEvent {
    pub fn which(&self) -> u32 {
        self.inner.get("which").as_::<u32>()
    }
}
