...
impl FromVal for ImageData {
    fn from_val(v: &emlite::Val) -> Self {
        ImageData { inner: emlite::Val::from_val(v) }
    }
...
impl AsRef<emlite::Val> for ImageData {
    fn as_ref(&self) -> &emlite::Val {
        &self.inner
    }
}
impl AsMut<emlite::Val> for ImageData {
...
impl ImageData {
    pub fn new0(data: Any, sw: u32) -> ImageData {
        Self {
            inner: emlite::Val::global("ImageData").new(&[data.into(), sw.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(data: Any, sw: u32, sh: u32) -> ImageData {
        Self {
            inner: emlite::Val::global("ImageData").new(&[data.into(), sw.into(), sh.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new2(data: Any, sw: u32, sh: u32, settings: ImageDataSettings) -> ImageData {
        Self {
            inner: emlite::Val::global("ImageData").new(&[data.into(), sw.into(), sh.into(), settings.into()]).as_::<emlite::Val>(),
        }
    }
