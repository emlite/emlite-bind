...
impl PaymentMethodChangeEvent {
    pub fn new0(type_: DOMString) -> PaymentMethodChangeEvent {
        Self {
            inner: emlite::Val::global("PaymentMethodChangeEvent").new(&[type_.into()]).as_::<PaymentRequestUpdateEvent>(),
        }
    }

    pub fn new1(type_: DOMString, event_init_dict: Any) -> PaymentMethodChangeEvent {
        Self {
            inner: emlite::Val::global("PaymentMethodChangeEvent").new(&[type_.into(), event_init_dict.into()]).as_::<PaymentRequestUpdateEvent>(),
        }
    }
