...
impl PointerEvent {
    pub fn new0(type_: DOMString) -> PointerEvent {
        Self {
            inner: emlite::Val::global("PointerEvent").new(&[type_.into()]).as_::<MouseEvent>(),
        }
    }

    pub fn new1(type_: DOMString, event_init_dict: Any) -> PointerEvent {
        Self {
            inner: emlite::Val::global("PointerEvent").new(&[type_.into(), event_init_dict.into()]).as_::<MouseEvent>(),
        }
    }
...
impl PointerEvent {
    pub fn get_coalesced_events(&self, ) -> Sequence<PointerEvent> {
        self.inner.call("getCoalescedEvents", &[]).as_::<Sequence<PointerEvent>>()
    }

}
impl PointerEvent {
    pub fn get_predicted_events(&self, ) -> Sequence<PointerEvent> {
        self.inner.call("getPredictedEvents", &[]).as_::<Sequence<PointerEvent>>()
    }
