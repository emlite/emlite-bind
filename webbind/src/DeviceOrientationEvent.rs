...
impl DeviceOrientationEvent {
    pub fn new0(type_: DOMString) -> DeviceOrientationEvent {
        Self {
            inner: emlite::Val::global("DeviceOrientationEvent").new(&[type_.into()]).as_::<Event>(),
        }
    }

    pub fn new1(type_: DOMString, event_init_dict: Any) -> DeviceOrientationEvent {
        Self {
            inner: emlite::Val::global("DeviceOrientationEvent").new(&[type_.into(), event_init_dict.into()]).as_::<Event>(),
        }
    }
...
impl DeviceOrientationEvent {
    pub fn request_permission0() -> Promise {
        emlite::Val::global("deviceorientationevent").call("requestPermission", &[]).as_::<Promise>()
    }

    pub fn request_permission1(absolute: bool) -> Promise {
        emlite::Val::global("deviceorientationevent").call("requestPermission", &[absolute.into(), ]).as_::<Promise>()
    }
