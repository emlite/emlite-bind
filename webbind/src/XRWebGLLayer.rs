...
impl AsRef<emlite::Val> for XRWebGLLayer {
    fn as_ref(&self) -> &emlite::Val {
        &self.inner
    }
}
impl AsMut<emlite::Val> for XRWebGLLayer {
...
impl XRWebGLLayer {
    pub fn new0(session: XRSession, context: Any) -> XRWebGLLayer {
        Self {
            inner: emlite::Val::global("XRWebGLLayer").new(&[session.into(), context.into()]).as_::<XRLayer>(),
        }
    }

    pub fn new1(session: XRSession, context: Any, layer_init: Any) -> XRWebGLLayer {
        Self {
            inner: emlite::Val::global("XRWebGLLayer").new(&[session.into(), context.into(), layer_init.into()]).as_::<XRLayer>(),
        }
    }
...
impl XRWebGLLayer {
    pub fn get_native_framebuffer_scale_factor(session: XRSession) -> f64 {
        emlite::Val::global("xrwebgllayer").call("getNativeFramebufferScaleFactor", &[session.into(), ]).as_::<f64>()
    }
