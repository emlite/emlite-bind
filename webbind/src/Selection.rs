...
impl FromVal for Selection {
    fn from_val(v: &emlite::Val) -> Self {
        Selection { inner: emlite::Val::from_val(v) }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
...
impl Selection {
    pub fn get_composed_ranges0(&self, ) -> Sequence<StaticRange> {
        self.inner.call("getComposedRanges", &[]).as_::<Sequence<StaticRange>>()
    }

    pub fn get_composed_ranges1(&self, options: GetComposedRangesOptions) -> Sequence<StaticRange> {
        self.inner.call("getComposedRanges", &[options.into(), ]).as_::<Sequence<StaticRange>>()
    }
...
impl Selection {
    ...
    pub fn modify3(&self, alter: DOMString, direction: DOMString, granularity: DOMString) -> Undefined {
