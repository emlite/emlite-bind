...
impl FromVal for IDBObjectStore {
    fn from_val(v: &emlite::Val) -> Self {
        IDBObjectStore { inner: emlite::Val::from_val(v) }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
...
impl IDBObjectStore {
    ...
    pub fn put1(&self, value: Any, key: Any) -> IDBRequest {
        self.inner.call("put", &[value.into(), key.into(), ]).as_::<IDBRequest>()
    }
...
impl IDBObjectStore {
    ...
    pub fn add1(&self, value: Any, key: Any) -> IDBRequest {
        self.inner.call("add", &[value.into(), key.into(), ]).as_::<IDBRequest>()
    }

}
impl IDBObjectStore {
    pub fn delete(&self, query: Any) -> IDBRequest {
        self.inner.call("delete", &[query.into(), ]).as_::<IDBRequest>()
    }
...
impl IDBObjectStore {
    pub fn create_index0(&self, name: DOMString, key_path: Any) -> IDBIndex {
        self.inner.call("createIndex", &[name.into(), key_path.into(), ]).as_::<IDBIndex>()
    }

    pub fn create_index1(&self, name: DOMString, key_path: Any, options: IDBIndexParameters) -> IDBIndex {
        self.inner.call("createIndex", &[name.into(), key_path.into(), options.into(), ]).as_::<IDBIndex>()
    }

}
impl IDBObjectStore {
    pub fn delete_index(&self, name: DOMString) -> Undefined {
        self.inner.call("deleteIndex", &[name.into(), ]).as_::<Undefined>()
    }
