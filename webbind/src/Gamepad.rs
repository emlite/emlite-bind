...
impl FromVal for Gamepad {
    fn from_val(v: &emlite::Val) -> Self {
        Gamepad { inner: emlite::Val::from_val(v) }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
...
impl Gamepad {
    pub fn buttons(&self) -> FrozenArray<GamepadButton> {
        self.inner.get("buttons").as_::<FrozenArray<GamepadButton>>()
    }
...
impl Gamepad {
    pub fn haptic_actuators(&self) -> FrozenArray<GamepadHapticActuator> {
        self.inner.get("hapticActuators").as_::<FrozenArray<GamepadHapticActuator>>()
    }
