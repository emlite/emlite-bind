...
impl FromVal for ReportingObserver {
    fn from_val(v: &Any) -> Self {
        ReportingObserver { inner: Any::from_val(v) }
    }
...
impl ReportingObserver {
    ...
    pub fn new0(callback: &Function) -> ReportingObserver {
        Self {
            inner: Any::global("ReportingObserver").new(&[callback.into()]).as_::<Any>(),
        }
    }
    ...
    pub fn new1(callback: &Function, options: &ReportingObserverOptions) -> ReportingObserver {
        Self {
            inner: Any::global("ReportingObserver").new(&[callback.into(), options.into()]).as_::<Any>(),
        }
    }
