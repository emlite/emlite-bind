...
impl FromVal for LayoutChild {
    fn from_val(v: &emlite::Val) -> Self {
        LayoutChild { inner: emlite::Val::from_val(v) }
    }
    fn take_ownership(v: emlite::env::Handle) -> Self {
        Self::from_val(&emlite::Val::take_ownership(v))
    }
...
impl LayoutChild {
    pub fn layout_next_fragment(&self, constraints: LayoutConstraintsOptions, break_token: ChildBreakToken) -> Promise {
        self.inner.call("layoutNextFragment", &[constraints.into(), break_token.into(), ]).as_::<Promise>()
    }
