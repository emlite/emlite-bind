...
impl FromVal for CookieStoreManager {
    fn from_val(v: &Any) -> Self {
        CookieStoreManager { inner: Any::from_val(v) }
    }
...
impl CookieStoreManager {
    ...
    pub fn subscribe(&self, subscriptions: &TypedArray<CookieStoreGetOptions>) -> Promise<Undefined> {
        self.inner.call("subscribe", &[subscriptions.into(), ]).as_::<Promise<Undefined>>()
    }
}
impl CookieStoreManager {
    ...
    pub fn get_subscriptions(&self, ) -> Promise<TypedArray<CookieStoreGetOptions>> {
        self.inner.call("getSubscriptions", &[]).as_::<Promise<TypedArray<CookieStoreGetOptions>>>()
    }
}
impl CookieStoreManager {
    /// The unsubscribe method.
    /// [`CookieStoreManager.unsubscribe`](https://developer.mozilla.org/en-US/docs/Web/API/CookieStoreManager/unsubscribe)
    pub fn unsubscribe(&self, subscriptions: &TypedArray<CookieStoreGetOptions>) -> Promise<Undefined> {
        self.inner.call("unsubscribe", &[subscriptions.into(), ]).as_::<Promise<Undefined>>()
    }
}
