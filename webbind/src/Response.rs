...
impl FromVal for Response {
    fn from_val(v: &emlite::Val) -> Self {
        Response { inner: emlite::Val::from_val(v) }
    }
...
impl Response {
    pub fn new0() -> Response {
        Self {
            inner: emlite::Val::global("Response").new(&[]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(body: Any) -> Response {
        Self {
            inner: emlite::Val::global("Response").new(&[body.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new2(body: Any, init: Any) -> Response {
        Self {
            inner: emlite::Val::global("Response").new(&[body.into(), init.into()]).as_::<emlite::Val>(),
        }
    }
...
impl Response {
    pub fn error() -> Response {
        emlite::Val::global("response").call("error", &[]).as_::<Response>()
    }

}
impl Response {
    pub fn redirect0(url: USVString) -> Response {
        emlite::Val::global("response").call("redirect", &[url.into(), ]).as_::<Response>()
    }

    pub fn redirect1(url: USVString, status: u16) -> Response {
        emlite::Val::global("response").call("redirect", &[url.into(), status.into(), ]).as_::<Response>()
    }
