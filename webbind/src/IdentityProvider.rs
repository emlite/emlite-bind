...
impl FromVal for IdentityProvider {
    fn from_val(v: &Any) -> Self {
        IdentityProvider { inner: Any::from_val(v) }
    }
...
impl IdentityProvider {
    ...
    pub fn close() -> Undefined {
        Any::global("IdentityProvider").call("close", &[]).as_::<Undefined>()
    }
}
impl IdentityProvider {
    /// The resolve method.
    ...
    pub fn resolve0(token: &JsString) -> Promise<Undefined> {
        Any::global("IdentityProvider").call("resolve", &[token.into(), ]).as_::<Promise<Undefined>>()
    }
    /// The resolve method.
    /// [`IdentityProvider.resolve`](https://developer.mozilla.org/en-US/docs/Web/API/IdentityProvider/resolve)
    pub fn resolve1(token: &JsString, options: &IdentityResolveOptions) -> Promise<Undefined> {
        Any::global("IdentityProvider").call("resolve", &[token.into(), options.into(), ]).as_::<Promise<Undefined>>()
    }
}
impl IdentityProvider {
    ...
    pub fn get_user_info(config: &IdentityProviderConfig) -> Promise<TypedArray<IdentityUserInfo>> {
        Any::global("IdentityProvider").call("getUserInfo", &[config.into(), ]).as_::<Promise<TypedArray<IdentityUserInfo>>>()
    }
}
