...
impl XRHitTestOptionsInit {
    pub fn entity_types(&self) -> Sequence<XRHitTestTrackableType> {
        self.inner.get("entityTypes").as_::<Sequence<XRHitTestTrackableType>>()
    }
...
impl XRSession {
    pub fn enabled_features(&self) -> FrozenArray<DOMString> {
        self.inner.get("enabledFeatures").as_::<FrozenArray<DOMString>>()
    }
...
impl XRSession {
    pub fn update_target_frame_rate(&self, rate: f32) -> Promise {
        self.inner.call("updateTargetFrameRate", &[rate.into(), ]).as_::<Promise>()
    }

}
impl XRSession {
    pub fn request_reference_space(&self, type_: XRReferenceSpaceType) -> Promise {
        self.inner.call("requestReferenceSpace", &[type_.into(), ]).as_::<Promise>()
    }
...
impl XRSession {
    pub fn persistent_anchors(&self) -> FrozenArray<DOMString> {
        self.inner.get("persistentAnchors").as_::<FrozenArray<DOMString>>()
    }

}
impl XRSession {
    pub fn restore_persistent_anchor(&self, uuid: DOMString) -> Promise {
        self.inner.call("restorePersistentAnchor", &[uuid.into(), ]).as_::<Promise>()
    }

}
impl XRSession {
    pub fn delete_persistent_anchor(&self, uuid: DOMString) -> Promise {
        self.inner.call("deletePersistentAnchor", &[uuid.into(), ]).as_::<Promise>()
    }
...
impl XRSession {
    pub fn request_hit_test_source(&self, options: XRHitTestOptionsInit) -> Promise {
        self.inner.call("requestHitTestSource", &[options.into(), ]).as_::<Promise>()
    }

}
impl XRSession {
    pub fn request_hit_test_source_for_transient_input(&self, options: XRTransientInputHitTestOptionsInit) -> Promise {
        self.inner.call("requestHitTestSourceForTransientInput", &[options.into(), ]).as_::<Promise>()
    }
...
impl XRSession {
    ...
    pub fn request_light_probe1(&self, options: XRLightProbeInit) -> Promise {
        self.inner.call("requestLightProbe", &[options.into(), ]).as_::<Promise>()
    }
