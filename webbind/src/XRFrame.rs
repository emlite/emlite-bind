...
impl FromVal for XRFrame {
    fn from_val(v: &emlite::Val) -> Self {
        XRFrame { inner: emlite::Val::from_val(v) }
    }
...
impl XRFrame {
    pub fn create_anchor(&self, pose: XRRigidTransform, space: XRSpace) -> Promise {
        self.inner.call("createAnchor", &[pose.into(), space.into(), ]).as_::<Promise>()
    }
...
impl XRFrame {
    pub fn fill_joint_radii(&self, joint_spaces: Sequence<XRJointSpace>, radii: Float32Array) -> bool {
        self.inner.call("fillJointRadii", &[joint_spaces.into(), radii.into(), ]).as_::<bool>()
    }

}
impl XRFrame {
    pub fn fill_poses(&self, spaces: Sequence<XRSpace>, base_space: XRSpace, transforms: Float32Array) -> bool {
        self.inner.call("fillPoses", &[spaces.into(), base_space.into(), transforms.into(), ]).as_::<bool>()
    }

}
impl XRFrame {
    pub fn get_hit_test_results(&self, hit_test_source: XRHitTestSource) -> Sequence<XRHitTestResult> {
        self.inner.call("getHitTestResults", &[hit_test_source.into(), ]).as_::<Sequence<XRHitTestResult>>()
    }

}
impl XRFrame {
    pub fn get_hit_test_results_for_transient_input(&self, hit_test_source: XRTransientInputHitTestSource) -> Sequence<XRTransientInputHitTestResult> {
        self.inner.call("getHitTestResultsForTransientInput", &[hit_test_source.into(), ]).as_::<Sequence<XRTransientInputHitTestResult>>()
    }
