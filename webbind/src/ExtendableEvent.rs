...
impl FromVal for ExtendableEvent {
    fn from_val(v: &emlite::Val) -> Self {
        ExtendableEvent { inner: Event::from_val(v) }
    }
...
impl ExtendableEvent {
    pub fn new0(type_: DOMString) -> ExtendableEvent {
        Self {
            inner: emlite::Val::global("ExtendableEvent").new(&[type_.into()]).as_::<Event>(),
        }
    }

    pub fn new1(type_: DOMString, event_init_dict: Any) -> ExtendableEvent {
        Self {
            inner: emlite::Val::global("ExtendableEvent").new(&[type_.into(), event_init_dict.into()]).as_::<Event>(),
        }
    }
