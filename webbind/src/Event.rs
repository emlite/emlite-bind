...
impl FromVal for Event {
    fn from_val(v: &emlite::Val) -> Self {
        Event { inner: emlite::Val::from_val(v) }
    }
...
impl Event {
    pub fn new0(type_: DOMString) -> Event {
        Self {
            inner: emlite::Val::global("Event").new(&[type_.into()]).as_::<emlite::Val>(),
        }
    }

    pub fn new1(type_: DOMString, event_init_dict: Any) -> Event {
        Self {
            inner: emlite::Val::global("Event").new(&[type_.into(), event_init_dict.into()]).as_::<emlite::Val>(),
        }
    }
...
impl Event {
    pub fn composed_path(&self, ) -> Sequence<EventTarget> {
        self.inner.call("composedPath", &[]).as_::<Sequence<EventTarget>>()
    }
