...
impl PaymentRequest {
    pub fn new0(method_data: Sequence<PaymentMethodData>, details: Any) -> PaymentRequest {
        Self {
            inner: emlite::Val::global("PaymentRequest").new(&[method_data.into(), details.into()]).as_::<EventTarget>(),
        }
    }

    pub fn new1(method_data: Sequence<PaymentMethodData>, details: Any, options: Any) -> PaymentRequest {
        Self {
            inner: emlite::Val::global("PaymentRequest").new(&[method_data.into(), details.into(), options.into()]).as_::<EventTarget>(),
        }
    }
...
impl PaymentRequest {
    ...
    pub fn show1(&self, details_promise: Promise) -> Promise {
        self.inner.call("show", &[details_promise.into(), ]).as_::<Promise>()
    }
...
impl PaymentRequest {
    pub fn secure_payment_confirmation_availability() -> Promise {
        emlite::Val::global("paymentrequest").call("securePaymentConfirmationAvailability", &[]).as_::<Promise>()
    }
